...
Just to quickly note. Now that we have our IDP connected to our ZTNA platform to complete authentication we want to ensure as we assign permissions to networks or applications on an as needed basis only, this follows the principal of least privilege as this is another essential pillar to Zero Trust. Throughout the demonstration we will be applying policy to the user to only access specific applications or destination addresses over specific known ports used by our application.
